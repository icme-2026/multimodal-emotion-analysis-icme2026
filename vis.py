...
def main_worker(gpu, args):
    ...
    def evaluate(self, eval_loader=None, args=None):
        ...
        metrics = {'eval/loss': total_loss / total_num, 'eval/top-1-acc': top1, 'eval/top-5-acc': top5}
        if args is not None and getattr(args, 'report_macro_f1', False):
            macro_f1 = f1_score(y_true, y_pred, average='macro')
            metrics['eval/macro-f1'] = macro_f1
            metrics['eval/per-class-f1'] = f1_score(y_true, y_pred, average=None).tolist()
        self.ema.restore()
        self.model.train()
        return metrics
...
if __name__ == "__main__":
    ...
    parser.add_argument('--report_macro_f1', type=str2bool, default=True,
                        help='whether to log macro-F1/per-class F1 during eval')
